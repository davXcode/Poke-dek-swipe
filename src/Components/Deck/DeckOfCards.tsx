...
const DeckOfCards: React.FC<DeckofCards> = ({ data }) => {
  ...
  useEffect(() => {
    if (pokeArray) {
      if (cardIndex < pokeArray.length) {
        if (
          getIDStringfromURL(pokeArray[cardIndex].url) !== stateActivePokemonID
        ) {
  ...
  useEffect(() => {
    if (data) {
      const filteredResult = data.filter((pokemon) =>
        pokemon.name.includes(stateSearch)
      );
        ...
        >
          <div className="relative text-gray-400 focus-within:text-black">
            ...
            <input
              ...
              placeholder="pokemon pokemon dimana kamu"
              autoComplete="off"
